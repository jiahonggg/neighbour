...
class helpdetailViewController: UITableViewController,UITextFieldDelegate{
    
    var helps:help?
    let namelabel:UILabel = {
        let labell = UILabel()
        labell.font = labell.font.fontWithSize(20)
        return labell
    }()
    ...
    let topview:UIView = {
        let view = UIView(frame: CGRectMake(0,0,UIScreen.mainScreen().bounds.size.width,250))
        return view
    }()
    ...
    func setupview(){
        ...
        discribetextview.snp_makeConstraints { (make) in
            ...
            make.right.equalTo(namelabel.snp_right).offset(-4)
            make.height.equalTo(150)
        }
        discribetextview.text = helps?.detaildiscribe
        namelabel.text = helps?.title
    }
