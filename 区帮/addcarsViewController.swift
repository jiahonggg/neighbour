...
class addcarsViewController: UIViewController , UITextViewDelegate,UIImagePickerControllerDelegate,UINavigationControllerDelegate{
    
    let titletextfield:UITextField = {
        let textfield = UITextField()
        textfield.placeholder = "拼车去哪？"
        return textfield
    }()
    let numbertextfield:UITextField = {
        let textfield = UITextField()
        textfield.placeholder = "拼车人数"
        textfield.backgroundColor = UIColor.whiteColor()
        return textfield
    }()
    let timetextfield:UITextField = {
        let textfield = UITextField()
        textfield.placeholder = "拼车时间"
        textfield.backgroundColor = UIColor.whiteColor()
        return textfield
    }()
    let addresstextfield:UITextField = {
        let textfield = UITextField()
        textfield.placeholder = "上车地址"
        textfield.backgroundColor = UIColor.whiteColor()
        return textfield
    }()
    ...
    func sendthings(){
        ...
        let storageref = FIRStorage.storage().reference().child("cars").child("\(imagenName).png")
        if let uploadData = UIImagePNGRepresentation(self.addimageview.image!){
            storageref.putData(uploadData, metadata: nil, completion: { (metadata, error) in
                ...
                if let salesimageurl = metadata?.downloadURL()?.absoluteString{
                    ...
                    let ref = FIRDatabase.database().reference().child("cars").childByAutoId()
                    let things :[String: AnyObject] = ["title":title!,
                        ...
                        "timestamp":timestamp]
                    ref.setValue(things)
                    let usersaleref = FIRDatabase.database().reference().child("user-cars").child(fromid!)
                    let saleId = ref.key
