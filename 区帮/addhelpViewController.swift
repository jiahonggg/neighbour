...
class addhelpViewController: UIViewController , UITextViewDelegate{
    ...
    func sendthings(){
        ...
        let ref = FIRDatabase.database().reference().child("helps").childByAutoId()
        let things :[String: AnyObject] = ["title":title!,
                                           "detaildiscribe":detaildiscribe,
                                           "fromid":fromid!,
                                           "timestamp":timestamp]
        ref.setValue(things)
        let usersaleref = FIRDatabase.database().reference().child("user-helps").child(fromid!)
        let saleId = ref.key
    ...
    override func viewDidLoad() {
        ...
        self.navigationItem.title = "寻人帮忙"
