...
class addsportsViewController: UIViewController , UITextViewDelegate,UIImagePickerControllerDelegate,UINavigationControllerDelegate{
    
    let titletextfield:UITextField = {
        let textfield = UITextField()
        textfield.placeholder = "运动标题"
        return textfield
    }()
    let numbertextfield:UITextField = {
        let textfield = UITextField()
        textfield.placeholder = "运动人数"
        textfield.backgroundColor = UIColor.whiteColor()
        return textfield
    }()
    let timetextfield:UITextField = {
        let textfield = UITextField()
        textfield.placeholder = "运动时间"
        textfield.backgroundColor = UIColor.whiteColor()
        return textfield
    }()
    let addresstextfield:UITextField = {
        let textfield = UITextField()
        textfield.placeholder = "运动地址"
        textfield.backgroundColor = UIColor.whiteColor()
        return textfield
    }()
    ...
    func sendthings(){
        ...
        let storageref = FIRStorage.storage().reference().child("sports").child("\(imagenName).png")
        if let uploadData = UIImagePNGRepresentation(self.addimageview.image!){
            storageref.putData(uploadData, metadata: nil, completion: { (metadata, error) in
                ...
                if let salesimageurl = metadata?.downloadURL()?.absoluteString{
                    ...
                    let ref = FIRDatabase.database().reference().child("sports").childByAutoId()
                    let things :[String: AnyObject] = ["title":title!,
                        ...
                        "timestamp":timestamp]
                    ref.setValue(things)
                    let usersaleref = FIRDatabase.database().reference().child("user-sports").child(fromid!)
                    let saleId = ref.key
    ...
    override func viewDidLoad() {
        ...
        self.navigationItem.title = "运动邀约"
        setupui()
        super.viewDidLoad()
